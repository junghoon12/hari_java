/*
   group by 절
   - 특정 컬럼이나 값을 기준으로 해당 레코드를 묶어서 자료를
     관리할 때 사용함.
...
-- 급여 합계와, 부서별 인원 수, 부서별 평균 급여, 부서별
-- 최대 급여, 부서별 최소 급여를 구하여 화면에 보여주세요.
-- 단, 급여 합계를 기준으로 내림차순으로 정렬하여 화면에
-- 보여주세요.
...
/*
   ...
   - View 는 테이블에 저장하기 위란 물리적인 공간이 필요가 없음.
...
-- 읽기 전용우로 만드는 방법
-- ==> View 만들 때 퀴리문 맨 마지막에 with read only 문구 추가;
...
/*
   ...
   - 트랜잭션 사용 시 트랜잭션을 제어하기 위한 명령어.
     1) commit : 모든 작업을 정상적으로 처리하겠다고 확정하는
                 명령어. 트랜잭션(insert, update, delete) 작업의
                 내용을 실제 DB에 반영.
                 이전에 있던 데이터에 update 현상이 발생을 함.
                 모든 사용자가 변경된 데이터의 결과를 볼 수 있음.
     2) rollback : 작업 중에 문제가 발생했을 때 트랜잭션 처리
                   과정에서 발생한 변경 사항을 취소하여 이전 상태로
                   되돌리는 명령어.
                   트랜잭션(insert, update, delete) 작업의
                   내용을 취소함. 이전에 commit 한 곳까지만
                   복구가 됨.
*/
...
-- 주의사항) 만약 where 조건절이 누락이 되면 테이블 전체 데이터가
--          수정이 이루어짐.
...
update student set
        major = '체육학과', phone = '010-7777-7890',
        addr = '서울시 서초구' where hakbun = '2023_005';
